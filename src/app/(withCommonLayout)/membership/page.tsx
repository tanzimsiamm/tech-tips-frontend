...
const membershipPackages = [
  {
    ...
    colorClass: "text-blue-500", // Tailwind class for text color
    bgColorClass: "bg-blue-500", // Tailwind class for button background
...
const Membership = () => {
  ...
  const [selectedMembership, setSelectedMembership] = useState<any>(null); // Renamed for clarity
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-black p-4 sm:p-6 lg:p-8">
      <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white mb-8 sm:mb-12 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 w-full max-w-6xl">
        {membershipPackages?.map((pack) => (
          <div
            key={pack.name}
            className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 sm:p-8 flex flex-col justify-between border border-gray-200 dark:border-gray-700 transition-transform duration-200 hover:scale-[1.02]"
          >
            <div>
            ...
            <button
              ...
              className={`w-full py-3 px-4 text-white font-semibold rounded-full ${pack.bgColorClass} ${pack.hoverBgColorClass} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${pack.colorClass.replace("text-", "")} transition-colors duration-200 dark:focus:ring-offset-gray-800`}
            >
              Subscribe Now
            </button>
          </div>
