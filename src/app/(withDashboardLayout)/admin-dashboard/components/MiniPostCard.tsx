    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 sm:p-6 w-full mx-auto mb-4 border border-gray-200 dark:border-gray-700">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center">
          <section className="group relative mr-3">
            <Link href={`/profile/${authorInfo?.authorEmail}`}>
              <Image
                ...
                src={authorInfo?.image || 'https://i.ibb.co/VtP9tF6/default-user-image.png'}
                alt="User Avatar"
              />
            </Link>
          ...
          <div>
            <Link href={`/profile/${authorInfo?.authorEmail}`} className="font-bold text-gray-900 dark:text-white hover:underline text-lg">
              {authorInfo?.name}
            </Link>
        ...
        <div className="flex items-center space-x-3 text-gray-600 dark:text-gray-400">
          {isPremium && <MdStars className="text-blue-500 text-2xl" title="Premium Content" />}

          {(user?.email === authorInfo?.authorEmail || user?.role === 'admin') && (
            <>
              <button
      ...
      <p className="text-gray-800 dark:text-gray-200 mb-4 text-base leading-relaxed font-semibold">
        {title}
        <Link href={`/details/${_id}`} className="text-blue-500 dark:text-blue-400 hover:underline pl-2">
          See more...
        </Link>
      </p>

      {images && images.length > 0 && (
        <Link href={`/details/${_id}`} className="block rounded-xl overflow-hidden">
          <Image
            width={300}
            height={300}
            alt='Post image'
      ...
      <div className="flex justify-between items-center mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
        <div className="flex items-center space-x-6 text-gray-600 dark:text-gray-400">
          <VoteSection postId={_id as string} userId={user?._id as string} votes={votes ?? 0} voters={voters ?? []} />

          <Link href={`/details/${_id}#comments`} className="flex items-center space-x-2 text-gray-600 dark:text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 transition-colors duration-200">
